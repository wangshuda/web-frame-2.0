...
public abstract class ImportSupport extends BodyTagSupport 
	...
	// Added by wangshuda 2010/12/02 START
	private Log log = LogFactory.getLog(this.getClass());
	
	private boolean urlIsExist = true;
	protected String urlExistVar = "importUrlExist";
	public void setUrlExistVar(String urlExistVar) {
		this.urlExistVar = urlExistVar;
	}
	// Added by wangshuda 2010/12/02 END
    //*********************************************************************
    // Public constants
    ...
    if (context != null) {
    	servletCtx = pageContext.getServletContext().getContext(context);
    }
    else {
    	servletCtx = pageContext.getServletContext();
    }
    ...
    String stripedUrl = stripSession(targetUrl);
    InputStream inputStream = null;
    try {
   	 inputStream = servletCtx.getResourceAsStream(stripedUrl);
        if(inputStream == null) {
       	 log.debug(new StringBuffer("Can not find the file: ").append(stripedUrl));
       	 urlIsExist = false;
       	 pageContext.setAttribute(urlExistVar, false, scope);
       	 return SKIP_BODY;
        }
        else {
        	urlIsExist = true;
        	pageContext.setAttribute(urlExistVar, true, scope);
        }
    }
    finally {
   	 if(inputStream != null) {
